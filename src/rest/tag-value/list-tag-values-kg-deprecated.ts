...
/**
 * DEPRECATED correlation tag-value search backed by the V2 Knowledge Graph.
 * Retained for the default (non-experimental) path until the REST
 * `/v1/tags/values` backend in `list-tag-values.ts` is promoted to GA.
 * Prefer `listTagValues`.
 */
export async function listTagValuesKGDeprecated({
  config,
  match,
  mode = "semantic",
  limit,
